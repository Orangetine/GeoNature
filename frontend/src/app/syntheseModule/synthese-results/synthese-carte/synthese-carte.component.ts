...
export class SyntheseCarteComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  private prepareCriteriaValues() {
    ...
    let combinedStyles = { ...defaultStyles, ...this.getOriginStyle(), ...defaultCriteriaStyles };
  ...
  layerEvent(feature, layer, idSyntheseIds) {
    layer.on({
      click: (e) => {
        ...
        console.log('layerEvent', this.selectedCriteria.code)
        if (this.selectedCriteria.code == this.areaAggregationCriteriaCode) {
  ...
  onEachFeature(feature, layer) {
    ...
    if (this.selectedCriteria.code == this.areaAggregationCriteriaCode) {
      ...
      this.setCriteriaStyle(layer, feature.properties.observations);
    }
  ...
  private setCriteriaStyle(layer, featureObsList) {
    //console.log("featureObsList", featureObsList)
    const field = this.selectedCriteria.field;
    const type = this.selectedCriteria.type;
    const values = (featureObsList[field]) ? featureObsList[field] : [];

    let featureCriteriaValue = (values.length > 1) ? '*' : values[0];
    if (type == 'nomenclatures' || featureCriteriaValue == '*') {
      layer.setStyle(this.selectedCriteria.preparedMarkerStyles[featureCriteriaValue]);
      ...
      for (let i = 0; i < classesNbr; i++) {
        ...
        if (i != lastIndex) {
          if (featureCriteriaValue > classeValue) {
            layer.setStyle(this.selectedCriteria.preparedMarkerStyles[classeValue]);
          }
        } else {
          layer.setStyle(this.selectedCriteria.preparedMarkerStyles[classeValue]);
        }
  ...
  private toggleStyleFromMap(feature, layer) {
    // Restore initial style
    if (this.selectedLayers.length > 0) {
      this.selectedLayers.forEach((layer) => {
        (layer as L.GeoJSON).setStyle(this.getOriginStyle());
      });
    }
  ...
  private getOriginStyle() {
    let originStyle = (this.selectedCriteria.code == this.areaAggregationCriteriaCode)
